...
if not BOT_TOKEN or ADMIN_ID == 0:
    raise RuntimeError("BOT_TOKEN yoki ADMIN_ID yo‘q. Koyeb env variables ga qo‘ying.")
...
async def show_anime(target: Message | CallbackQuery, anime_id: int):
    ...
    meta = []
    if a.get("year"): meta.append(f"📅 Yili: {a['year']}")
    if a.get("country"): meta.append(f"🌍 Davlati: {a['country']}")
    if a.get("language"): meta.append(f"🗣 Tili: {a['language']}")
    if a.get("genres"): meta.append(f"🎭 Janri: {a['genres']}")
    if a.get("description"): meta.append(f"\n📝 {a['description']}")

    lock_line = "🔒 Kod bilan yopilgan" if a.get("is_locked", 0) == 1 else "✅ Ochiq"
    txt = f"🎬 <b>{a['title']}</b>\n🆔 ID: <code>{a['id']}</code>\n{lock_line}\n" + "\n".join(meta)

    kb = seasons_kb(anime_id, seasons, is_admin=is_admin((target.from_user.id if isinstance(target, CallbackQuery) else target.from_user.id))) if seasons else None

    if isinstance(target, Message):
        await target.answer(txt, reply_markup=kb, disable_web_page_preview=True)
    else:
        await target.message.edit_text(txt, reply_markup=kb, disable_web_page_preview=True)
...
async def start_cmd(msg: Message):
    ...
    if is_admin(msg.from_user.id):
        ...
        text += "🆔 Sizning admin ID: <code>{}</code>\n".format(ADMIN_ID)
...
async def send_episode(call: CallbackQuery):
    ...
    anime_id = int(anime_id); season_no = int(season_no); episode_no = int(episode_no)
    ...
    # Video yoki document: file_id turiga qarab ishlaydi
    try:
...
# --- Admin: Fasl qo‘shish (anime ichidan) ---
...
async def admin_add_season_cb(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AddSeason.anime_id)
...
async def add_season_no(msg: Message, state: FSMContext):
    ...
    await state.clear()
    await msg.answer("✅ Fasl qo‘shildi. /start <code>{}</code> qilib tekshir.".format(anime_id))

# --- Admin: Qism qo‘shish (tez) ---
...
async def ep_video(msg: Message, state: FSMContext):
    ...
    if mode == "auto":
        ep_no = await db.next_episode_no(anime_id, season_no)
    else:
        ep_no = data["episode_no"]

    a = await db.get_anime(anime_id)
    title = a["title"] if a else "Media"
    caption = f"🎬 {title}\n📺 {season_no}-FASL • {ep_no}-qism"

    await db.add_or_replace_episode(anime_id, season_no, ep_no, file_id, caption=caption)

    me = await bot.get_me()
    deep = f"https://t.me/{me.username}?start={anime_id}"

    await msg.answer(
        f"✅ Saqlandi: ID <code>{anime_id}</code> | {season_no}-fasl | {ep_no}-qism\n"
        f"🔗 Link: {deep}\n\n"
        "Auto bo‘lsa: keyingi videoni yuboraver — o‘zi ketma-ket qo‘shadi.",
        disable_web_page_preview=True
    )

# --- Admin: Kod bilan yopish ---
...
async def edit_anime_id(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "Qaysi maydon?\n"
        "<code>title</code>, <code>year</code>, <code>country</code>, <code>language</code>, <code>genres</code>, <code>description</code>\n\n"
        "Masalan: <code>genres</code>"
    )
...
async def edit_anime_field(msg: Message, state: FSMContext):
    ...
    if field not in {"title","year","country","language","genres","description"}:
        return await msg.answer("Xato maydon. Masalan: <code>title</code> yoki <code>genres</code>")
...
async def edit_anime_value(msg: Message, state: FSMContext):
    ...
    await state.clear()
    await msg.answer("✅ Tahrirlandi. /start <code>{}</code> qilib tekshir.".format(anime_id))

# --- Admin: Epizod tahriri (fasl sahifasidan) ---
@dp.callback_query(F.data.startswith("admin:edit_ep:"))
async def edit_ep_from_season(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return await call.answer("Admin emassiz.", show_alert=True)
    _, _, anime_id, season_no = call.data.split(":")
    await state.set_state(EditEpisode.anime_id)
    await state.update_data(anime_id=int(anime_id), season_no=int(season_no))
    await state.set_state(EditEpisode.episode_no)
    await call.message.answer("✏️ Qaysi epizodni tahrirlaysiz? (qism raqami)")
    await call.answer()

@dp.message(EditEpisode.episode_no)
async def edit_ep_no(msg: Message, state: FSMContext):
    if not msg.text.isdigit():
        return await msg.answer("Faqat raqam.")
    ep_no = int(msg.text)
    data = await state.get_data()
    anime_id = int(data["anime_id"]); season_no = int(data["season_no"])
    exists = await db.get_episode(anime_id, season_no, ep_no)
    if not exists:
        return await msg.answer("Bunday qism topilmadi.")
    await state.update_data(episode_no=ep_no)
    await state.set_state(EditEpisode.action)
    await msg.answer("Nimani tahrirlaymiz?\n1) <code>caption</code>\n2) <code>file</code> (video almashtirish)")

@dp.message(EditEpisode.action)
async def edit_ep_action(msg: Message, state: FSMContext):
    act = msg.t