...
def get_episode_buttons(anime, title, page=0):
    ...
    # Admin uchun "🛠️ Tahrirlash" menyusi
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Epizodlar / o‘chirish / tahrirlash qolgan kodlari avvalgidek
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Epizod tahrirlash yoki qo‘shish
    ...
    if "editing_field" in context.user_data and "edit_title" in context.user_data:
        ...
        db[title][field] = text
        save_data(db)
        await update.message.reply_text(f"✅ {title} uchun {field} yangilandi!")
        context.user_data.clear()
