...
async def st_country(msg: Message, state: FSMContext):
    ...
    await msg.answer("🗣 Tili? (bo‘sh bo‘lsa '-')")
...
async def admin_add_season_cb(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("➕ Qaysi fasl raqamini qo‘shamiz? (1/2/3...)")
    await call.answer()
...
# --- Admin: Epizod tahrirlash (fasldan) ---
...
async def edit_ep_from_season(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("✏️ Qaysi epizod? (qism raqamini yubor)")
    await call.answer()
...
async def edit_ep_no(msg: Message, state: FSMContext):
    ...
    await msg.answer("Nimani o‘zgartiramiz?\n1) <code>caption</code>\n2) <code>file</code>\n\nJavob: caption yoki file")


@dp.message(EditEpisode.action)
async def edit_ep_action(msg: Message, state: FSMContext):
    act = msg.text.strip().lower()
    if act not in ("caption", "file"):
        return await msg.answer("Faqat <code>caption</code> yoki <code>file</code> yoz.")
    await state.update_data(action=act)

    if act == "caption":
        await state.set_state(EditEpisode.new_caption)
        return await msg.answer("Yangi caption yubor (bo‘sh bo‘lsa '-' yubor):")

    await state.set_state(EditEpisode.waiting_new_video)
    await msg.answer("Yangi video yoki document yubor:")


@dp.message(EditEpisode.new_caption)
async def edit_ep_caption(msg: Message, state: FSMContext):
    data = await state.get_data()
    anime_id = int(data["anime_id"])
    season_no = int(data["season_no"])
    ep_no = int(data["episode_no"])

    cap = "" if msg.text.strip() == "-" else msg.text.strip()
    await db.update_episode_caption(anime_id, season_no, ep_no, cap)
    await state.clear()
    await msg.answer("✅ Caption yangilandi.")


@dp.message(EditEpisode.waiting_new_video)
async def edit_ep_file(msg: Message, state: FSMContext):
    if not is_admin(msg.from_user.id):
        return

    file_id = msg.video.file_id if msg.video else (msg.document.file_id if msg.document else None)
    if not file_id:
        return await msg.answer("Video yoki document yubor.")
    ...
    await db.update_episode_file(anime_id, season_no, ep_no, file_id)
    await state.clear()
    await msg.answer("✅ Fayl (video) almashtirildi.")


# --- Admin: Post shabloni ---
@dp.callback_query(F.data == "admin:post")
async def post_start(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return await call.answer("Admin emassiz.", show_alert=True)
    await state.set_state(PostTemplate.anime_id)
    await call.message.answer("📢 Qaysi Anime ID uchun post shabloni? (raqam yubor)")
    await call.answer()


@dp.message(PostTemplate.anime_id)
async def post_anime_id(msg: Message, state: FSMContext):
    if not msg.text.isdigit():
        return await msg.answer("Faqat raqam.")
    anime_id = int(msg.text)
    a = await db.get_anime(anime_id)
    if not a:
        return await msg.answer("Topilmadi.")
    await state.update_data(anime_id=anime_id)
    await state.set_state(PostTemplate.season_no)
    await msg.answer("Qaysi fasl? (masalan 1)")


@dp.message(PostTemplate.season_no)
async def post_season(msg: Message, state: FSMContext):
    if not msg.text.isdigit():
        return await msg.answer("Faqat raqam.")
    season_no = int(msg.text)
    data = await state.get_data()
    ...
    await state.clear()
    await msg.answer(text, link_preview_options=LPO_OFF)
